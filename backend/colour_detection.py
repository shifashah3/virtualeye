...
class ColorDetector:
    ...
    3. dominant cluster
    4. LAB palette matching
    5. optional visible color ranking without changing primary-color logic
    """
    ...
    def _get_color_clusters(self, pixels, max_colors=5):
        """
        Used only for SECONDARY visible colors.
        Does NOT replace your main color logic.
        """
        if len(pixels) == 0:
            return []
        ...
        for idx, count in enumerate(counts):
            clusters.append({
                "rgb": tuple(int(x) for x in centers[idx]),
                "count": int(count),
            })
    ...
    def _build_visible_colors(self, pixels, primary_name, primary_rgb, max_colors=5):
        """
        Preserve current primary color exactly.
        Then add secondary colors from clustering.
        """
        total = len(pixels) if len(pixels) > 0 else 1
        clusters = self._get_color_clusters(pixels, max_colors=max_colors * 2)
        ...
        for cluster in clusters:
            ...
            if color_name == primary_name:
                ...
                grouped[color_name]["count"] += count

        visible_colors = [{
            "rank": 1,
            "name": primary_name,
            "rgb": primary_rgb,
            "hex": self._rgb_to_hex(primary_rgb),
            "percentage": round((primary_count / total) * 100, 2) if total > 0 else 100.0,
        }]
        ...
        secondary.sort(key=lambda x: x["percentage"], reverse=True)

        for idx, item in enumerate(secondary[:max_colors - 1], start=2):
            item["rank"] = idx
            visible_colors.append(item)

        return visible_colors[:max_colors]
    ...
    def detect_color_simple(self, image_bytes, bbox=None, history_key="default", max_colors=5):
        image = Image.open(io.BytesIO(image_bytes)).convert("RGB")
        image.thumbnail((640, 640))
        image_np = np.array(image)
        ...
        visible_colors = self._build_visible_colors(
            filtered_pixels,
            primary_name=raw_color,
            primary_rgb=rgb,
        ...
        return {
            "rgb": rgb,
            "hex": self._rgb_to_hex(rgb),
            "name": raw_color,            # primary color stays EXACTLY as before
            "stable_name": stable_color,  # kept for debugging
